...
import app.mosia.nexus.domain.model.common.Visualization.VisualizationConfig
import app.mosia.nexus.domain.model.common.{EntityId, Quaternion, Vector3}
import app.mosia.nexus.domain.model.task.SuccessCriteria
import zio.Duration
...
object Goal:
  ...
  /** 目标条件 */
  enum GoalCondition:
    case ReachPosition(target: Vector3, tolerance: Double, orientation: Option[Quaternion] = None)
    case FollowPath(waypoints: List[Vector3], maxDeviation: Double, speedLimit: Option[Double] = None)
    ...
    case StayInArea(center: Vector3, radius: Double)
  ...
  case class GoalMonitoring(
    ...
    visualization: Option[VisualizationConfig] = None
  )
