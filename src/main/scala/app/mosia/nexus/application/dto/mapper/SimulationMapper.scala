package app.mosia.nexus.application.dto.mapper

import app.mosia.nexus.application.dto.model.scene.{MaterialDto, ObstacleDto, SceneConfigDto, SensorDto}
import app.mosia.nexus.domain.model.common.Color
import app.mosia.nexus.domain.model.scene.*
...
object SimulationMapper:
  ...
  def toObstacle(dto: ObstacleDto): Either[String, Obstacle] =
    ...
    yield Obstacle(
      id = ObstacleId(java.util.UUID.randomUUID().toString),
      obstacleType = obstacleType,
      position = dto.position,
      rotation = dto.rotation.getOrElse(app.mosia.nexus.domain.model.common.Quaternion(1.0, 0.0, 0.0, 0.0)),
  ...
  def toSensor(dto: SensorDto): Either[String, Sensor] =
    ...
    yield Sensor(
      id = SensorId(java.util.UUID.randomUUID()),
      sensorType = sensorType,
      position = dto.position,
      orientation = dto.orientation.getOrElse(app.mosia.nexus.domain.model.common.Quaternion(1.0, 0.0, 0.0, 0.0)),
      config = SensorConfig(
        frequency = Some(30.0),
        range = Some(10.0),
        fov = Some(90.0),
