...
final class HealthCheckServiceLive(
  ...
  private def checkDatabase(): AppTask[ComponentHealth] =
    ...
    (for {
      ...
      (pgValid, tsValid) <- checkOne(default.ds, "PostgreSQL")
        .zipPar(checkOne(timescale.ds, "TimescaleDB"))
      responseTime <- Clock.currentTime(TimeUnit.MILLISECONDS).map(_ - start)
    } yield {
      if pgValid && tsValid then
        ComponentHealth(
        ...
        val issues = List(
          ...
          if !tsValid then Some("TimescaleDB") else None
        ).flatten.mkString(", ")
        ComponentHealth(
...
object HealthCheckServiceLive:
  val live: ZLayer[PostgresDataSource & TimescaleDataSource, Nothing, HealthCheckServiceLive] =
    ZLayer.fromFunction(HealthCheckServiceLive(_, _))
