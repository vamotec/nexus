...
final class SmartRoutingStrategy(
  ...
  override def selectCluster(context: RoutingContext): AppTask[String] =
    (for
      ...
      selected <- redis
        .get[String](context.userId.value.toString)
        .flatMap:
      ...
      // 4. 更新缓存
      _ <- redis.set[String](key = context.userId.value.toString, value = selected)
...
object SmartRoutingStrategy:
  val live: ZLayer[ClusterRegistry & RedisService, Nothing, SmartRoutingStrategy] =
