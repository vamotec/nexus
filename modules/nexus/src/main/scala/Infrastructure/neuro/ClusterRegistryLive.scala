...
final class ClusterRegistryLive(
  config: AppConfig,
  manager: NeuroConnectionManager
) extends ClusterRegistry:
  ...
  private def initializeMetadata(config: AppConfig): Map[String, ClusterMetadata] =
    config.neuro.clusters.map { case (clusterId, clusterCfg) =>
...
object ClusterRegistryLive:
  val live: ZLayer[AppConfig & NeuroConnectionManager, Nothing, ClusterRegistry] =
    ZLayer.fromFunction(ClusterRegistryLive(_, _))
