...
import domain.services.infra.{ClusterRegistry, ClusterRoutingStrategy, NeuroClient}
...
final class NeuroClientLive(
  ...
  private def withManualRouting[A](
    ...
    for {
      ...
      stub <- manager.getStub(clusterId)
      res <- ZIO
        .fromFuture(_ => f(stub))
        .mapError(toAppError)
...
object NeuroClientLive:
  ...
  val live: ZLayer[NeuroConnectionManager & ClusterRegistry, Nothing, NeuroClient] =
    ZLayer.scoped:
      for
        manager <- ZIO.service[NeuroConnectionManager]
