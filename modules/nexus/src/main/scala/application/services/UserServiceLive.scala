...
final class UserServiceLive(userRepo: UserRepository, eventPublisher: DomainEventPublisher, redis: RedisService)
    extends UserService:
  override def createUser(email: String, plainPassword: String): AppTask[User] =
    for
      // Check if username already exists
      _ <- userRepo.findByEmail(email).flatMap {
        case Some(_) => ZIO.fail(UserAlreadyExists(email))
...
object UserServiceLive:
  val live: ZLayer[UserRepository & DomainEventPublisher & RedisService, Nothing, UserService] =
